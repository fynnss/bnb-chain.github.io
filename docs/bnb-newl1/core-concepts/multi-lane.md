...
Lane-to-address mappings and reserved shares live in the on-chain `LaneRegistry` contract (address `0x0000000000000000000000000000000000003000`), which is governed — changes go through the same [governance process](../governance/overview.md) as any other protocol parameter.
...
- Lane occupancy is currently measured by each transaction's **declared gas limit**, not its actual gas used — a consequence of blocks being ordered before they're executed (see [Consensus](./consensus.md)). In practice this means a transaction can occupy more of a lane's quota than it ultimately consumes.
...
3. Remember that protection is per-address, not per-function — anything sent to that address consumes the same reserved quota.
