---
title: FAQ - Beyond Chain
---

# FAQ

**Is Beyond Chain EVM-compatible?**
...
**Is Beyond Chain live on mainnet?**
...
No. Beyond Chain's account abstraction is native, enforced by consensus rather than by a deployed EntryPoint contract, so no bundler or paymaster contract is involved. See [Account Abstraction](./core-concepts/account-abstraction.md).
...
No. The circuits are unaudited, and the deployment runs on an imported trusted setup rather than a Beyond Chain ceremony. Both are open items before mainnet. See the warning in [Privacy](./core-concepts/privacy.md).

**Why doesn't `eth_getProof` work?**
Beyond Chain commits state with a cumulative hash accumulator instead of a Merkle-Patricia trie, so there's no trie-based proof to serve. See [State DB](./core-concepts/state-db.md).
...
BNB, bridged in from BNB Smart Chain. It is the same gas asset as BSC and opBNB.
